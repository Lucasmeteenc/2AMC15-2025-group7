import torch
import numpy as np
import matplotlib.pyplot as plt
from pathlib import Path
from environments.medium_delivery_env import MediumDeliveryEnv
from ppo_network import NetworkFactory
from ppo_agent import PPOConfig
from maps import MAIL_DELIVERY_MAPS

# Ignore all warnings
import warnings
warnings.filterwarnings("ignore")

torch.serialization.add_safe_globals([PPOConfig])

def load_model(model_path: str, state_size: int, action_size: int, device: torch.device):
    actor = NetworkFactory.create_actor(state_size, action_size).to(device)
    checkpoint = torch.load(model_path, map_location=device, weights_only=False)
    actor.load_state_dict(checkpoint["actor_state_dict"])
    actor.eval()
    return actor

def evaluate_and_heatmap(model_path, map_name="default", trials=20, save_path="heatmap.png"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    env = MediumDeliveryEnv(map_config=MAIL_DELIVERY_MAPS[map_name], render_mode=None, seed=42)
    state_size = env.observation_space.shape[0]
    action_size = env.action_space.n
    actor = load_model(model_path, state_size, action_size, device)

    # Use a 100x100 grid for the heatmap
    grid_size = 100
    map_w, map_h = env.map_size
    heatmap = np.zeros((grid_size, grid_size), dtype=np.int32)

    for episode in range(trials):
        obs_np, _ = env.reset()
        done = False
        step_count = 0
        total_score = 0
        while not done:
            # Get agent's continuous position
            x = getattr(env, "agent_x", None)
            y = getattr(env, "agent_y", None)
            if step_count == 0:
                print(f"Initial position: x={x:.4f}, y={y:.4f}")
            # Map to heatmap grid
            grid_x = int(np.clip(x / map_w * (grid_size - 1), 0, grid_size - 1))
            grid_y = int(np.clip(y / map_h * (grid_size - 1), 0, grid_size - 1))
            heatmap[grid_y, grid_x] += 1
            obs = torch.from_numpy(np.asarray(obs_np, dtype=np.float32)).to(device)
            logits = actor(obs)
            dist = torch.distributions.Categorical(logits=logits)
            action = dist.sample().item()
            next_obs_np, score, terminated, truncated, _ = env.step(action)
            total_score += score
            done = terminated or truncated
            obs_np = next_obs_np
            step_count += 1
        print(f"Episode {episode + 1}/{trials} - Steps: {step_count}, Score: {total_score}")

    env.close()

    plt.figure(figsize=(8, 6))
    plt.imshow(heatmap, cmap="hot", interpolation="nearest", origin="lower", extent=[0, map_w, 0, map_h])
    plt.title(f"Heatmap of Visited Spaces ({trials} episodes)")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.colorbar(label="Visit Count")
    plt.savefig(save_path)
    plt.close()
    print(f"Heatmap saved to {save_path}")

if __name__ == "__main__":
    evaluate_and_heatmap(
        model_path="final_model_cb3tpg9d.pth",
        map_name="default",
        trials=5,
        save_path="ppo_agent_heatmap.png"
    )
