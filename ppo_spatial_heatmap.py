...
def render_map(ax, map_config):
    ...
    for x0, y0, x1, y1 in map_config["obstacles"]:
        ax.add_patch(
            Rectangle((x0, y0), x1 - x0, y1 - y0, color="gray", alpha=0.5)
        )
    ...
    ax.plot(start_x, start_y, marker="o", markersize=6, color="lightblue", label="Start")

    # Delivery point (red X)
    deliv_x, deliv_y = map_config["delivery"]
    off = 0.2 * SCALE
    ax.plot([deliv_x - off, deliv_x + off], [deliv_y - off, deliv_y + off], color="red", linewidth=2 * SCALE)
    ax.plot([deliv_x - off, deliv_x + off], [deliv_y + off, deliv_y - off], color="red", linewidth=2 * SCALE)


def create_environment(cfg: PPOConfig):
    """Instantiate the MediumDelivery environment with a fixed seed."""
    return MediumDeliveryEnv(map_config=MAIL_DELIVERY_MAPS[cfg.map_name], render_mode=None, seed=42)
...
def load_model(path: str, cfg: PPOConfig, device: torch.device):
    ...
    # Case B: the file is *just* the actor state_dict
        ...
        raise PPOError("Unrecognised checkpoint format for PPO model – expecting either a CheckpointManager file or a raw state_dict containing layer weights.")
...
def evaluate_and_spatial_heatmap(
    ...
    for ep in range(trials):
        ...
        logging.info("Episode %d/%d - steps: %d, score: %.2f", ep + 1, trials, step_cnt, score_sum)
